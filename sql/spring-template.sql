...
CREATE TABLE `oper_log`  (
  ...
  `resp_result` varchar(5000) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '返回参数',
...
CREATE TABLE `user`  (
  ...
  `expire_date` date NOT NULL DEFAULT (DATE_ADD(CURDATE(), INTERVAL 90 DAY)) COMMENT '密码过期日期',
  `is_deleted` bigint(0) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否逻辑删除，0-未删除，时间戳-已删除',
  `create_by` bigint(0) UNSIGNED NULL DEFAULT NULL COMMENT '创建人',
  `create_time` datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '创建时间',
  `update_by` bigint(0) UNSIGNED NULL DEFAULT NULL COMMENT '修改人',
